...
export async function setBookedCount(sessionId, count) {
  await kv.set(`seats:${sessionId}`, count);
}
...
// 잔여석 계산 (실제 예약 기록 + 선점 기준)
export async function getRemainSeats(sessionId, totalSeats) {
  const [reservations, locked] = await Promise.all([
    getReservations(),
    getLockedSeats(sessionId),
  ]);
  const booked = reservations
    .filter(r => r.sessionId === sessionId && (r.payStatus === '입금확인' || r.payStatus === '미입금' || r.payStatus === '현장결제예정'))
