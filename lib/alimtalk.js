...
async function sendAlimtalk(to, text, templateCode, buttons = []) {
  if (!templateCode) {
    console.warn('[ALIMTALK] 템플릿 코드 미설정 — 발송 생략');
    return false;
  }
  if (!to) {
    console.warn('[ALIMTALK] 수신번호 없음 — 발송 생략');
    return false;
  }
...
미입금 시 예약이 자동 취소됩니다.`}`;
  ...
  return sendAlimtalk(phone, text, templateCode, buttons);
}
...
// ============================================================
// 변경 완료 — 고객에게 발송 (변경티켓 포함)
// ============================================================
export async function sendChangeCompleteAlimtalk({
  name, phone, resNum, session, quantity, perfName, customText, btn1Name, templateCode,
}) {
  const baseUrl = (process.env.TICKET_BASE_URL || 'https://ticket-alarm-manage.vercel.app/ticket.html').replace('/ticket.html', '');
  const btnUrl  = baseUrl + '/ticket.html?res=' + encodeURIComponent(resNum);
  ...
  if (customText) {
    text = customText
      ...
      .replace(/#{회차}/g,     session)
      .replace(/#{매수}/g,     String(quantity))
      .replace(/#{티켓링크}/g, btnUrl);
  } else {
    text = `[${perfName}] 예약 변경이 완료되었습니다.
...
// 취소 완료 — 고객에게 발송
// ============================================================
export async function sendCancelCompleteAlimtalk({
  name, phone, resNum, session, perfName, customText, templateCode,
}) {
  let text = '';
  if (customText) {
    text = customText
...
취소 처리가 완료되었습니다.
문의사항은 카카오톡 채널로 연락해 주세요.`;
  }
  return sendAlimtalk(phone, text, templateCode);
}
...
export async function sendCancelRequestAlimtalk({
  adminPhone, resNum, name, session, total, reason, perfName, customText, templateCode,
}) {
  const amountStr = total === 0 ? '무료' : Number(total).toLocaleString() + '원';
  let text = '';
  if (customText) {
    text = customText
      .replace(/#{공연명}/g,     perfName)
      ...
      .replace(/#{회차}/g,       session)
      .replace(/#{결제금액}/g,   amountStr)
      .replace(/#{취소사유}/g,   reason || '');
  } else {
    text =
`[${perfName}] 취소/변경 신청이 접수되었습니다.
...
관리자 페이지에서 확인해 주세요.`;
  ...
  return sendAlimtalk(adminPhone, text, templateCode);
}
