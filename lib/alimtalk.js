...
const TPL_CANCEL   = process.env.ALIMTALK_TEMPLATE_CANCEL   || TPL_CANCEL;
...
미입금 시 예약이 자동 취소됩니다.`}`;
  ...
  return sendAlimtalk(phone, text, TPL_RESERVE, buttons);
}
...
공연 30분 전까지 입장해 주세요.`;
  ...
  return sendAlimtalk(phone, text, TPL_TICKET, buttons);
}
...
모바일 티켓을 미리 준비해 주세요.`;
  ...
  return sendAlimtalk(phone, text, TPL_REMINDER, buttons);
}
