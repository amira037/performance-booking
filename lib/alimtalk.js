// lib/alimtalk.js — 솔라피 카카오 알림톡 발송

import crypto from 'crypto';

const SOLAPI_API_KEY    = process.env.SOLAPI_API_KEY;
const SOLAPI_API_SECRET = process.env.SOLAPI_API_SECRET;
const SENDER_PHONE      = process.env.SENDER_PHONE;
const KAKAO_CHANNEL_ID  = process.env.KAKAO_CHANNEL_ID;
// 카카오톡 채널 채팅 URL — 채널 개설 후 실제 ID로 교체
// 형식: https://pf.kakao.com/_채널영문ID/chat
const KAKAO_CHAT_URL    = process.env.KAKAO_CHAT_URL || 'https://pf.kakao.com/_xlUqEX/chat';

function generateSignature(timestamp) {
  const salt = Math.random().toString(36).substring(2);
  const msg  = timestamp + salt;
  const sig  = crypto.createHmac('sha256', SOLAPI_API_SECRET).update(msg).digest('hex');
  return { salt, signature: sig };
}

// 버튼 포함 알림톡 발송
async function sendAlimtalk(to, text, templateCode, buttons = []) {
  try {
    const timestamp = new Date().toISOString();
    const { salt, signature } = generateSignature(timestamp);

    const kakaoOptions = {
      pfId: KAKAO_CHANNEL_ID,
      templateCode,
    };
    if (buttons.length > 0) kakaoOptions.buttons = buttons;

    const res = await fetch('https://api.solapi.com/messages/v4/send', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `HMAC-SHA256 apiKey=${SOLAPI_API_KEY}, date=${timestamp}, salt=${salt}, signature=${signature}`,
      },
      body: JSON.stringify({
        message: {
          to: to.replace(/-/g, ''),
          from: SENDER_PHONE,
          text,
          type: 'ATA',
          kakaoOptions,
        },
      }),
    });

    const result = await res.json();
    if (result.errorCode) {
      console.error('알림톡 오류:', result.errorMessage);
      return false;
    }
    return true;
  } catch(e) {
    console.error('솔라피 오류:', e.message);
    return false;
  }
}

// ============================================================
// 템플릿 1 — 예약 접수 + 입금 안내
// 버튼: 입금 완료 알리기 (채널 채팅) / 예매 취소 신청
// ============================================================
export async function sendReservationAlimtalk({
  name, phone, resNum, session, ticketType, quantity,
  total, perfName, account, accountHolder, kakaopayLink, cancelUrl,
}) {
  const isFree    = total === 0;
  const amountStr = isFree ? '무료' : Number(total).toLocaleString() + '원';

  let paymentBlock = '';
  if (isFree) {
    paymentBlock = '본 티켓은 무료입니다. 별도 입금이 필요하지 않습니다.';
  } else {
    paymentBlock = '[방법 1] 계좌이체\n' + account + '\n예금주: ' + accountHolder;
    if (kakaopayLink) {
      paymentBlock += '\n\n[방법 2] 카카오페이 송금\n' + kakaopayLink + '\n송금 시 ' + amountStr + '을 입력해 주세요.';
    }
  }

  const text =
`[${perfName}] 예약이 접수되었습니다.

■ 예약번호: ${resNum}
■ 예매자: ${name}
■ 회차: ${session}
■ 유형: ${ticketType} × ${quantity}매
■ 결제금액: ${amountStr}

━━━━━━━━━━━━━━━━━━
${paymentBlock}${isFree ? '' : `
입금기한: 예약 후 24시간 이내
입금 확인 후 모바일 티켓이 발송됩니다.
미입금 시 예약이 자동 취소됩니다.`}`;

  // 버튼 구성 (무료는 입금 버튼 불필요)
  const buttons = [];
  if (!isFree) {
    buttons.push({
      buttonName: '입금 완료 알리기',
      buttonType: 'WL',              // 웹링크
      linkMo: KAKAO_CHAT_URL,        // 모바일
      linkPc: KAKAO_CHAT_URL,        // PC
    });
  }
  if (cancelUrl) {
    buttons.push({
      buttonName: '예매 취소 신청',
      buttonType: 'WL',
      linkMo: cancelUrl,
      linkPc: cancelUrl,
    });
  }

  return sendAlimtalk(phone, text, 'TEMPLATE_01', buttons);
}

// ============================================================
// 템플릿 2 — 입금 확인 + 티켓 발송
// 버튼: 모바일 티켓 보기
// ============================================================
export async function sendTicketAlimtalk({
  name, phone, resNum, session, quantity, needProof, perfName, ticketUrl,
}) {
  const proofNote = needProof ? '\n⚠️ 입장 시 할인 증빙 서류를 지참해 주세요.' : '';
  const text =
`[${perfName}] 입금이 확인되었습니다.

■ 예약번호: ${resNum}
■ 예매자: ${name}
■ 회차: ${session}
■ 매수: ${quantity}매${proofNote}

공연 30분 전까지 입장해 주세요.`;

  const buttons = [{
    buttonName: '🎫 모바일 티켓 보기',
    buttonType: 'WL',
    linkMo: ticketUrl,
    linkPc: ticketUrl,
  }];

  return sendAlimtalk(phone, text, 'TEMPLATE_02', buttons);
}

// ============================================================
// 템플릿 3 — 공연 당일 리마인드
// 버튼: 모바일 티켓 보기
// ============================================================
export async function sendReminderAlimtalk({
  name, phone, resNum, session, perfName, ticketUrl,
}) {
  const text =
`[${perfName}] 오늘 공연 안내입니다.

■ 예매자: ${name}
■ 회차: ${session}
■ 예약번호: ${resNum}

공연 30분 전까지 입장해 주세요.
모바일 티켓을 미리 준비해 주세요.`;

  const buttons = ticketUrl ? [{
    buttonName: '🎫 모바일 티켓 보기',
    buttonType: 'WL',
    linkMo: ticketUrl,
    linkPc: ticketUrl,
  }] : [];

  return sendAlimtalk(phone, text, 'TEMPLATE_03', buttons);
}

// ============================================================
// 취소 신청 — 관리자에게 발송
// ============================================================
export async function sendCancelRequestAlimtalk({
  adminPhone, resNum, name, session, total, reason, perfName,
}) {
  const text =
`[${perfName}] 취소 신청이 접수되었습니다.

■ 예약번호: ${resNum}
■ 예매자: ${name}
■ 회차: ${session}
■ 결제금액: ${total === 0 ? '무료' : Number(total).toLocaleString() + '원'}
■ 취소 사유: ${reason}

관리자 페이지에서 취소 처리해 주세요.`;

  // 관리자 알림은 템플릿 없이 SMS로 발송 (또는 별도 템플릿 등록)
  return sendAlimtalk(adminPhone, text, 'TEMPLATE_CANCEL');
}
